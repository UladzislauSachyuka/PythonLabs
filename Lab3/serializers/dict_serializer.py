import inspect
from serializers.constants import nonetype, moduletype, codetype, celltype, \
...
class DictSerializer:
    ...
    def to_dict(cls, obj, is_inner_func=False):
        ...
        if type(obj) is codetype:
            ...
            for (key, value) in inspect.getmembers(obj):  # достает в виде списка кортежей все атрибуты (имя атрибута и его значение)
        ...
        if type(obj) is celltype:  # в celltype сохраняются значения переменных замыкания (вышли из области видимости)
        ...
        if inspect.isroutine(obj):    # проверяет, является ли объект функцией или методом
            ...
            # скомпилированный код тела функции или метода
    ...
    def from_dict(cls, obj, is_dict=False):
        ...
        if type(obj) not in (dict, list):
            ...
            if obj_type == codetype.__name__:
                return codetype(*[cls.from_dict(obj_source[prop]) for prop in CODE_PROPS]) # распаковка листа
